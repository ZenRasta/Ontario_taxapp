...
// Import updated types - ensure NonRegisteredAccount is exported from types/api.ts
import { ScenarioInput, SpouseInfo, NonRegisteredAccount } from '../types/api';

// --- State Interfaces ---
// Define state using string types for form inputs
interface SpouseInfoState {
    age: string;
    rrsp_balance: string;
    employment_income: string;
    pension_income: string;
    cpp_oas_income: string; // Representing combined CPP/OAS for simplicity in form
...
interface FormDataState {
    ...
    rrsp_balance: string; // Ensure present
    // Section 2
    employment_income: string;
    pension_type: string;
    pension_income: string;
    ...
    // Section 3
    has_spouse: boolean; // Use boolean for checkbox state
    ...
    // Section 5
    tfsa_balance: string;
    has_non_registered: boolean; // Use boolean for checkbox state
...
type FormErrors = {
    [key in keyof FormDataState]?: string; // Use keys from FormDataState
...
interface InputFormProps {
    onSubmit: (data: ScenarioInput) => void; // Expect fully typed ScenarioInput
    isLoading?: boolean;
}
...
const InputForm: React.FC<InputFormProps> = ({ onSubmit, isLoading = false }) => {
    ...
    const initialFormData: FormDataState = {
        ...
        tfsa_balance: '0', has_non_registered: false, non_registered_details: null, // Initialize has_non_registered
    ...
    // Removed showSpouseFields/showNonRegFields - use formData.has_spouse / formData.has_non_registered directly
    const [errors, setErrors] = useState<FormErrors>({});

    // --- Handlers ---
    const handleCheckboxChange = (event: ChangeEvent<HTMLInputElement>) => {
        const { name, checked } = event.target;
        // Clear potential errors for sections being toggled
        setErrors(prev => {
             const e = {...prev};
    ...
    // Generic handler for TextField inputs
    const handleTextFieldChange = (event: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
        const { name, value } = event.target;
        const errorKey = name as keyof FormErrors; // Name matches error key for TextField
        setErrors(prev => ({ ...prev, [errorKey]: undefined })); // Clear error

        if (name.startsWith('spouse_details.')) {
            const field = name.split('.')[1] as keyof SpouseInfoState;
            setFormData((prev) => ({ ...prev, spouse_details: prev.spouse_details ? { ...prev.spouse_details, [field]: value } : null, }));
        } else if (name.startsWith('non_registered_details.')) {
             const field = name.split('.')[1] as keyof NonRegState;
             setFormData((prev) => ({ ...prev, non_registered_details: prev.non_registered_details ? { ...prev.non_registered_details, [field]: value } : null }));
        } else {
    ...
    const validateForm = (): boolean => {
        ...
        // Validate fields section by section
        ...
        validateNumeric('rrsp_balance', formData.rrsp_balance, 'Your RRSP/RRIF Balance', true, 0);
        ...
        validateNumeric('other_investment_income', formData.other_investment_income, 'Other Invst Inc', true, 0);
        if (formData.has_spouse && formData.spouse_details) {
            ...
            validateNumeric('spouse_details.rrsp_balance', formData.spouse_details.rrsp_balance, 'Spouse RRSP', true, 0);
            validateNumeric('spouse_details.employment_income', formData.spouse_details.employment_income, 'Spouse Employ Inc', true, 0);
            validateNumeric('spouse_details.pension_income', formData.spouse_details.pension_income, 'Spouse Pension Inc', true, 0);
            validateNumeric('spouse_details.cpp_oas_income', formData.spouse_details.cpp_oas_income, 'Spouse CPP/OAS', true, 0);
            validateNumeric('spouse_details.investment_income', formData.spouse_details.investment_income, 'Spouse Invest Inc', true, 0);
        }
        ...
        if (formData.has_non_registered && formData.non_registered_details) { // Use has_non_registered
    ...
    const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
        ...
        if (validateForm()) {
            ...
            // Explicitly build the final object matching ScenarioInput type
            const scenarioData: ScenarioInput = {
                age: safeParseInt(formData.age) ?? 0,
                retirement_status: formData.retirement_status,
                retirement_age: safeParseInt(formData.retirement_age, true), // Optional -> null
                rrsp_balance: safeParseFloat(formData.rrsp_balance) ?? 0,
                ...
                spouse_details: null, // Initialize as null
                beneficiary_intent: formData.beneficiary_intent === "" ? null : formData.beneficiary_intent, // Map empty string to null if needed
                ...
                non_registered_details: null, // Initialize as null
                ...
                target_rrif_depletion_age: safeParseInt(formData.target_rrif_depletion_age, true), // Optional -> null
                province: formData.province,
                future_residence_intent: formData.future_residence_intent === "" ? null : formData.future_residence_intent,
                start_year: safeParseInt(formData.start_year, true), // Optional -> null
            };

            // Conditionally add spouse details if present
            if (formData.has_spouse && formData.spouse_details) {
                scenarioData.spouse_details = {
                    age: safeParseInt(formData.spouse_details.age) ?? 0,
                    rrsp_balance: safeParseFloat(formData.spouse_details.rrsp_balance) ?? 0,
             ...
             // Conditionally add non-registered details if present
            if (formData.has_non_registered && formData.non_registered_details) {
                 scenarioData.non_registered_details = {
